...
const Empresa = (props: RouteProps) => {
    ...
    const doar = () => {
        ...
        const idEmpresa = new URLSearchParams(props.location.search).get('idEmpresa');
        Api.post('/Transacao', {valor: state.valorDoacao, EmpresaId: +idEmpresa, UsuarioId: idUsuario})
        .then(res => {
            console.log(res);
            Api.get('/Empresa?id=' + idEmpresa + '&to=1&imagem=true')
            .then(res => {
    ...
    return (
        <div className={styles.paginaEmpresa}>
            ...
            <Dialog open={state.modalDoar}>
                ...
                <DialogContent>
                    <TextField value={state.valorDoacao} onChange={e => setState({...state, valorDoacao: +e.target.value})} label="Valor" type="number"/>
                </DialogContent>
                <DialogActions>
